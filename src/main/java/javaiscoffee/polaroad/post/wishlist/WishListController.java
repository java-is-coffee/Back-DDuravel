...
public class WishListController {
    ...
    @Operation(summary = "사용자의 위시리스트 목록 조회", description = "위시리스트 목록 조회하는 API")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "위시리스트에 포스트 삭제를 성공했을 경우"),
            @ApiResponse(responseCode = "404", description = "멤버가 존재하지 않는 경우")
    })
    ...
    @Operation(summary = "포스트 추가할 때 보여줄 위시리스트 목록 및 포함여부 조회", description = "포스트를 위시리스트에 추가할 때 보여줄 위시리스트 목록 및 포함여부 조회하는 API")
    ...
    @Operation(summary = "위시리스트에 들어있는 포스트 목록 조회", description = "위시리스트에 있는 포스트 목록 조회하는 API \n ## 현재 본인의 위시리스트 내용만 볼 수 있게 설정")
