...
public class JpaReviewRepository implements ReviewRepository{
    ...
    public Slice<Review> findReviewSlicedByPostId(Post post, Pageable pageable, ReviewStatus status) {
        ...
        // 페이징된 결과를 가져오는 쿼리
        TypedQuery<Review> query = em.createQuery("SELECT r FROM Review r WHERE r.post = :post AND r.status = :status ORDER BY r.goodNumber DESC, r.reviewId DESC", Review.class)
                .setParameter("post", post)
                .setParameter("status", status);
